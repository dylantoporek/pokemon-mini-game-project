...
win._ccScriptSettings.user.localCachedUser=cachedUser;function setStorage(){switch(ccao.privacy.law.toLowerCase()){case "gdpr":if(ccao.privacy.gdpr.Consent){try{window.localStorage.setItem("carbon_ccuid",win._ccScriptSettings.user.id);window.sessionStorage.setItem("carbon_ccsid",win._ccScriptSettings.session.id);}catch(err){console.debug('error:'+err.message);}}
