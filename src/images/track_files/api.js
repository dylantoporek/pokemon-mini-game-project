...
data.push(["customAttributes",customAttributes.join(";")]);return data;};function setStorage(store,key,value){switch(ccao.privacy.law.toLowerCase()){case "gdpr":if(ccao.privacy.gdpr.Consent){store.setItem(key,value);}
...
function storeMd5Hash(hash,label){hash=hash.toLowerCase();if(validateMd5Hash(hash)){var storageKey="cc-"+label+"-md5";setLocalStorage(storageKey,hash);if(ccaoSettings!==null)
pixelSync(getSyncUserUrl()+getSyncUrlParams(hash,false,label+"-md5"))();}
else{console.log("String is not a valid md5 hash");}}
function storeSha256Hash(hash,label){hash=hash.toLowerCase();if(validateSha256Hash(hash)){var storageKey="cc-"+label+"-sha256";setLocalStorage(storageKey,hash);if(ccaoSettings!==null)
...
pixelSync(getSyncUserUrl()+getSyncUrlParams(clientUserId,useSiteId))();};this.getUserId=function(){return userId;};this.recordEvent=function(eventName){if(eventName&&eventName.length>0){var data=buildEventMessage();ccao.sendMessage('custom-ce-'+eventName,data,true);}};this.refreshCarbon=function(){ccao.refreshGPTTargeting();ccao.refresh();};this.storeHashedEmail=function(algorithm,hash){var label="hem";if(algorithm.toLowerCase()=="md5"){storeMd5Hash(hash,label);}
else if(algorithm.toLowerCase()=="sha256"){storeSha256Hash(hash,label);}
else{console.log("Algorithm not supported: "+algorithm);}};this.storeHashedTelephone=function(algorithm,hash){var label="htel";if(algorithm.toLowerCase()=="md5"){storeMd5Hash(hash,label);}
else if(algorithm.toLowerCase()=="sha256"){storeSha256Hash(hash,label);}
